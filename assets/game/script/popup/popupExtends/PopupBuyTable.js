...
cc.Class({
    ...
    start () {
        // add event vao day
        this.node.on(PopupManager.PopupBuyTableConfig.ON_CHANGE_EVENT_CLICK, this.onHandleEvent, this);
    ...
    onHandleEvent(event){
        for(var i =0;i<this.listItemNode.length;i++){
            this.listItemNode[i].getComponent("ItemTable").activeEventClick(event.index);
        }
    },
    initData(){
        this.listItemNode = [];
        ...
        if(prefabInit){
            for(var i =0;i<PopupManager.PopupBuyTableConfig.NUMBER_ITEM;i++){
