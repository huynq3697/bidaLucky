...
cc.Class({
    ...
    onLoad () {
        // this.content.on('SELECT_ITEM_EVENT', (event)=>{
        //     if(typeof event.detail === 'function'){
        //         this.setSelectFilter(event.detail);
        //     }else{
        //         cc.warn('SELECT_ITEM_EVENT 事件的detail没有设置为选择器函数');
        //     }
        // });
    },
    ...
    update (dt) {
        ...
        if (this.content) {
              ...
              if (y === this._lastY || this.content.height <= this.mask.height || !this._initedFlag) {
                  return;
              }
              // 是否上滑
    ...
    refresh (infos, filter){
        ...
        const osize = this.content.getContentSize();
        this.content.setContentSize(cc.size(osize.width, height));
    ...
    _init (infos){
        ...
        if (this.itemCount < 1){
            // 没有要显示的节点
            this.content.setContentSize(cc.size(0, 0));
            return;
        }
        ...
        this.content.setContentSize(cc.size(width, height));
        this.content.setAnchorPoint(cc.v2(0.5, 1));
        this.content.x = 0;
        this.content.y = this.mask.height / 2;
        // 初始化第一个节点
        this._initRowItem(rowItem, 0);
        this._rowItems.push(rowItem);
        this._lastY = this.content.y;
        this._topIndex = 0;
        ...
        // cc.log(`一共创建了${this.rowItemCount * this.column}个Item实例`);
    },
    ...
    _outOfVisibleTop(rowItem){
        const wpos = this.content.convertToWorldSpaceAR(cc.v2(rowItem.x, rowItem.y - rowItem.height));
        const mpos = this.mask.convertToNodeSpaceAR(wpos);
        return mpos.y > this.mask.height / 2;
    },

    _outOfVisibleBottom(rowItem){
        const wpos = this.content.convertToWorldSpaceAR(rowItem.getPosition());
        const mpos = this.mask.convertToNodeSpaceAR(wpos);
        return mpos.y < -this.mask.height / 2;
    }
});
